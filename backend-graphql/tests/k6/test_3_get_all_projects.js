...
const GRAPHQL_QUERY = `
  query GetAllProjects($input: PaginationInput) {
  ...
  check(response, {
    ...
    'response has pagination structure': (r) => {
      try {
        ...
        return body.data?.projects?.hasOwnProperty('data') && 
               body.data?.projects?.hasOwnProperty('pagination') &&
               Array.isArray(body.data.projects.data);
      } catch (e) {
        return false;
      }
    },
    'projects have required fields': (r) => {
      try {
        const body = JSON.parse(r.body);
        const projects = body.data?.projects?.data;
    ...
    'pagination has required fields': (r) => {
      try {
        const body = JSON.parse(r.body);
        const pagination = body.data?.projects?.pagination;
...
export function handleSummary(data) {
  ...
  const summary = {
    test_name: 'GraphQL All Projects Load Test',
    endpoint: `/graphql`,
  ...
  const fileName = `../performance-analytics/summary/graphql/3_get_all_projects.json`;
  return {
