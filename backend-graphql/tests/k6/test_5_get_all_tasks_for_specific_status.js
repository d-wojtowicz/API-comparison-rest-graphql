...
const GRAPHQL_QUERY = `
  query GetTasksByStatus($statusId: ID!) {
    tasksByStatus(statusId: $statusId) {
      task_id
      task_name
      description
      status_id
  ...
  check(response, {
    ...
    'response is array': (r) => {
      try {
        ...
        return Array.isArray(body.data?.tasksByStatus);
      } catch (e) {
        return false;
      }
    },
    'tasks have required fields': (r) => {
      try {
        const body = JSON.parse(r.body);
        const tasks = body.data?.tasksByStatus;
        if (!Array.isArray(tasks)) return false;
        
        // If no tasks, that's valid
        if (tasks.length === 0) return true;
        
        // Check first task has required fields
        const task = tasks[0];
        return task.hasOwnProperty('task_id') && 
               task.hasOwnProperty('task_name') && 
               task.hasOwnProperty('description') &&
               task.hasOwnProperty('status_id') &&
    ...
    'tasks have correct status_id': (r) => {
      try {
        const body = JSON.parse(r.body);
        const tasks = body.data?.tasksByStatus;
        if (!Array.isArray(tasks)) return false;
        ...
        // Check all tasks have the correct status_id
        return tasks.every(task => task.status_id === STATUS_ID.toString());
      } catch (e) {
        return false;
      }
    },
...
export function handleSummary(data) {
  ...
  const summary = {
    test_name: 'GraphQL Tasks by Status Load Test',
    endpoint: `/graphql`,
    status_id: STATUS_ID,
  ...
  const fileName = `../performance-analytics/summary/graphql/5_get_all_tasks_for_specific_status.json`;
  return {
