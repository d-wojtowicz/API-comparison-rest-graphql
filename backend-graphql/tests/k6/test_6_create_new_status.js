  ...
  check(response, {
    ...
    'response has status_id': (r) => {
      try {
        ...
        return body.data?.createStatus?.hasOwnProperty('status_id') && 
               typeof body.data.createStatus.status_id === 'string';
      } catch (e) {
        return false;
      }
    },
    'response has correct status_name': (r) => {
      try {
        const body = JSON.parse(r.body);
        return body.data?.createStatus?.hasOwnProperty('status_name') && 
               body.data.createStatus.status_name === statusName;
      } catch (e) {
        return false;
      }
    },
    'status_id is valid': (r) => {
      try {
        const body = JSON.parse(r.body);
        const statusId = body.data?.createStatus?.status_id;
        return statusId && !isNaN(Number(statusId)) && Number(statusId) > 0;
      } catch (e) {
        return false;
      }
    },
...
export function handleSummary(data) {
  ...
  const summary = {
    test_name: 'GraphQL Create Status Load Test',
  ...
  const fileName = `../performance-analytics/summary/graphql/6_create_new_status.json`;
  return {
