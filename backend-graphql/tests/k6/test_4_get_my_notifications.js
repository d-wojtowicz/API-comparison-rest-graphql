  ...
  check(response, {
    ...
    'response has pagination structure': (r) => {
      try {
        ...
        return body.data?.myNotifications?.hasOwnProperty('data') && 
               body.data?.myNotifications?.hasOwnProperty('pagination') &&
               Array.isArray(body.data.myNotifications.data);
      } catch (e) {
        return false;
      }
    },
    'notifications have required fields': (r) => {
      try {
        const body = JSON.parse(r.body);
        const notifications = body.data?.myNotifications?.data;
        if (!notifications || !Array.isArray(notifications)) return false;
    ...
    'pagination has required fields': (r) => {
      try {
        const body = JSON.parse(r.body);
        const pagination = body.data?.myNotifications?.pagination;
        if (!pagination) return false;
        
        return pagination.hasOwnProperty('hasNextPage') && 
               pagination.hasOwnProperty('nextCursor') !== undefined &&
...
export function handleSummary(data) {
  ...
  const summary = {
    test_name: 'GraphQL My Notifications Load Test',
    endpoint: `/graphql`,
  ...
  const fileName = `../performance-analytics/summary/graphql/4_get_my_notifications.json`;
  return {
