  ...
  check(response, {
    ...
    'response has comment_id': (r) => {
      try {
        ...
        return body.data?.createTaskComment?.hasOwnProperty('comment_id') && 
               typeof body.data.createTaskComment.comment_id === 'string';
      } catch (e) {
        return false;
      }
    },
    'response has correct task_id': (r) => {
      try {
        const body = JSON.parse(r.body);
        return body.data?.createTaskComment?.hasOwnProperty('task_id') && 
               body.data.createTaskComment.task_id === TASK_ID.toString();
      } catch (e) {
        return false;
      }
    },
    'response has correct user_id': (r) => {
      try {
        const body = JSON.parse(r.body);
        return body.data?.createTaskComment?.hasOwnProperty('user_id') && 
               body.data.createTaskComment.user_id === '1'; // The user ID from the JWT token
      } catch (e) {
        return false;
      }
    },
    'response has correct comment_text': (r) => {
      try {
        const body = JSON.parse(r.body);
        return body.data?.createTaskComment?.hasOwnProperty('comment_text') && 
               body.data.createTaskComment.comment_text === commentText;
      } catch (e) {
        return false;
      }
    },
    'comment_id is valid': (r) => {
      try {
        const body = JSON.parse(r.body);
        const commentId = body.data?.createTaskComment?.comment_id;
        return commentId && !isNaN(Number(commentId)) && Number(commentId) > 0;
      } catch (e) {
        return false;
      }
    },
    'response has required fields': (r) => {
      try {
        const body = JSON.parse(r.body);
        const comment = body.data?.createTaskComment;
        return comment?.hasOwnProperty('comment_text') && 
...
export function handleSummary(data) {
  ...
  const summary = {
    test_name: 'GraphQL Create Comment Load Test',
  ...
  const fileName = `../performance-analytics/summary/graphql/8_create_comment_for_specific_task.json`;
  return {
