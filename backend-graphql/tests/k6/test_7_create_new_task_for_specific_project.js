...
export default function () {
  ...
  // Generate unique task name using timestamp and random string
  const timestamp = Date.now();
  const randomSuffix = randomString(8);
  const taskName = `test-task-${timestamp}-${randomSuffix}`;
  const description = `Test task description for load testing - ${timestamp}`;
  ...
  const payload = JSON.stringify({
    ...
    variables: {
      input: {
        ...
        priority: Math.floor(Math.random() * 5) + 1 // Random priority between 1-5
      }
    }
  });
  ...
  check(response, {
    ...
    'response has task_id': (r) => {
      try {
        ...
        return body.data?.createTask?.hasOwnProperty('task_id') && 
               typeof body.data.createTask.task_id === 'string';
      } catch (e) {
        return false;
      }
    },
    'response has correct task_name': (r) => {
      try {
        const body = JSON.parse(r.body);
        return body.data?.createTask?.hasOwnProperty('task_name') && 
               body.data.createTask.task_name === taskName;
      } catch (e) {
        return false;
      }
    },
    'response has correct project_id': (r) => {
      try {
        const body = JSON.parse(r.body);
        return body.data?.createTask?.hasOwnProperty('project_id') && 
               body.data.createTask.project_id === PROJECT_ID.toString();
      } catch (e) {
        return false;
      }
    },
    'response has correct status_id': (r) => {
      try {
        const body = JSON.parse(r.body);
        return body.data?.createTask?.hasOwnProperty('status_id') && 
               body.data.createTask.status_id === STATUS_ID.toString();
      } catch (e) {
        return false;
      }
    },
    'task_id is valid': (r) => {
      try {
        const body = JSON.parse(r.body);
        const taskId = body.data?.createTask?.task_id;
        return taskId && !isNaN(Number(taskId)) && Number(taskId) > 0;
      } catch (e) {
        return false;
      }
    },
    'response has required fields': (r) => {
      try {
        ...
        return task?.hasOwnProperty('task_name') && 
               task?.hasOwnProperty('description') &&
               task?.hasOwnProperty('priority') &&
               task?.hasOwnProperty('created_at') &&
               task?.hasOwnProperty('updated_at');
      } catch (e) {
        return false;
      }
    },
...
export function handleSummary(data) {
  ...
  const summary = {
    test_name: 'GraphQL Create Task Load Test',
  ...
  const fileName = `../performance-analytics/summary/graphql/7_create_task_for_specific_project.json`;
  return {
