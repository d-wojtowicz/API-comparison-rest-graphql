...
export default function () {
  const url = `${BASE_URL}/api/v1/statuses/${STATUS_ID}/tasks`;
  ...
  check(response, {
    ...
    'tasks have correct status_id': (r) => {
      try {
        ...
        return body.every(task => task.status_id === STATUS_ID);
      } catch (e) {
        return false;
      }
    },
...
export function handleSummary(data) {
  ...
  const summary = {
    test_name: 'REST Tasks by Status Load Test',
    endpoint: `/api/v1/statuses/{statusId}/tasks`,
    status_id: STATUS_ID,
  ...
  const fileName = `../performance-analytics/summary/rest/5_get_all_tasks_for_specific_status.json`;
  return {
